...
public class WorldGenTreeJungleTall extends WorldGenerator
{
	...
	public boolean generate(World world, Random rand, BlockPos pos) 
	{
    	...
    	if(root > 0f)
    	{
	    	for(int k = 0; k < 3; k++)
	    	{
	    		generateBranch(world, rand, pos.up(root), (120 * k) - 40 + rand.nextInt(80), 1.6f + rand.nextFloat() * 0.1f, root * 1.7f, 1f);
	    	}
    	}
    	...
    	for(int j = 0; j < branch; j++)
    	{
        	...
        	for(int m = 0; m < 1; m++)
        	{
            	generateLeaves(world, rand, pos.add(eX, eY, eZ), 4f, 1.5f);
        	}
    	}

...
public class WorldGenTreeBirch extends WorldGenerator
{
	private int startHeight;
	private int treeSize;
	
	public WorldGenTreeBirch(int start, int s)
	{
		startHeight = start;
		treeSize = s;
	}
    ...
    public boolean generate(World world, Random rand, BlockPos pos)
    {
    	...
    	{
    		world.setBlockState(pos, Blocks.log.getStateFromMeta(2), 0);
    		pos = pos.down();
    	}
    	...
    	for(i = 0; i < treeSize; i++)
    	{
    		if(rand.nextInt(1) == 0 && i < treeSize - 2)
    		{
        		...
        		buildBranch(world, rand, pos, dX, dZ, 1, i < treeSize - 2 ? 2 : 1); //i < treeSize - 4 ? 2 : 1
    		}
    		world.setBlockState(pos, Blocks.log.getStateFromMeta(2), 0);
    ...
    public void buildBranch(World world, Random rand, BlockPos pos, int dX, int dZ, int logLength, int leaveSize)
    {
    	...
    	{
        	world.setBlockState(pos.add(dX * m, 0, dZ * m), Blocks.log.getStateFromMeta(2), 0);
    	}
    }
    ...
    public void buildLeaves(World world, BlockPos pos)
    {
    	...
    	{
    		world.setBlockState(pos, Blocks.leaves.getStateFromMeta(2), 0);
    	}
    }
}
